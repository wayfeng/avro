...
#ifdef QAT_CODEC
struct codec_data_qz {
    QzSession_T sess;
    //QzSession_T comp_sess;
    //QzSession_T decomp_sess;
};

//#define codec_data_qz_comp_session(cd)   &((struct codec_data_qz *)cd)->comp_sess
//#define codec_data_qz_decomp_session(cd)   &((struct codec_data_qz *)cd)->decomp_sess
#define codec_data_qz_session(cd)   &((struct codec_data_qz *)cd)->sess

static int codec_qat(avro_codec_t codec)
{
    int rc = -1;
    /*
    QzSessionParams_T comp_params = {0};
    QzSessionParams_T decomp_params = {0};
    */

    codec->name = "qat";
    codec->type = AVRO_CODEC_QAT;
    codec->block_size = 0;
    codec->used_size = 0;
    codec->block_data = NULL;
    codec->codec_data = avro_new(sizeof(struct codec_data_qz));
    if (codec->codec_data == NULL) {
        avro_set_error("Cannot allocate memory for QAT sessions");
        return 1;
    }

    /*
    if (qzGetDefaults(&comp_params) != QZ_OK) {
        avro_set_error("Cannot get default parameters of QAT sessions");
        return 1;
    }
    ...
    rc = qzSetupSession(codec_data_qz_comp_session(codec->codec_data), NULL);
    if (rc != QZ_OK) {
        avro_set_error("Cannot setup QAT compress session");
        return 1;
    }
    rc = qzSetupSession(codec_data_qz_decomp_session(codec->codec_data), NULL);
    if (rc != QZ_OK) {
        avro_set_error("Cannot setup QAT decompress session");
        return 1;
    }
    */
    QzSession_T *sess = codec_data_qz_session(codec->codec_data);
    rc = qzInit(sess, 1);
    if (rc != QZ_OK) {
        avro_set_error("Cannot setup QAT session");
        avro_free(codec->codec_data, sizeof(struct codec_data_qz));
        return 1;
    }
    QzSessionParamsLZ4S_T params = {0};
    if (qzGetDefaultsLZ4S(&params) != QZ_OK) {
        avro_set_error("Cannot get default parameters of QAT sessions");
        return 1;
    }
    rc = qzSetupSessionLZ4S(codec_data_qz_session(codec->codec_data), &params);
    if (rc != QZ_OK) {
        avro_set_error("Cannot setup QAT session");
        avro_free(codec->codec_data, sizeof(struct codec_data_qz));
        return 1;
...
static int encode_qat(avro_codec_t c, void * data, int64_t len)
{
    size_t outlen = len+16; /* guess adding 16 extra bytes is OK, maybe not enough */

    if (!c->block_data) {
        //c->block_data = avro_malloc(outlen);
        c->block_data = qzMalloc(outlen, 0, PINNED_MEM);
        c->block_size = outlen;
    } else if (c->block_size < (int64_t)outlen) {
        qzFree(c->block_data);
        c->block_data = qzMalloc(outlen, 0, PINNED_MEM);
        c->block_size = outlen;
    }
    ...
    if (!c->block_data) {
        avro_set_error("Cannot allocate memory for QAT");
        return 1;
    }

    unsigned int last = 1;
    unsigned int used = outlen;
    int rc = qzCompress(codec_data_qz_session(c->codec_data),
            (unsigned char*)data, (uint32_t *)(&len),
            (unsigned char*)c->block_data, &used, last);
    if (rc != QZ_OK) {
        avro_set_error("Failed to compress %lu bytes with errno %d.\n", len, rc);
        return 1;
    }
    c->used_size = used;
...
static int decode_qat(avro_codec_t c, void * data, int64_t len)
{
    size_t outlen = len * 20; /* magic number from qatzip sample code */
    ...
    unsigned int used = outlen;
    unsigned int consumed = len;
    int rc = qzDecompress(codec_data_qz_session(c->codec_data),
            data, &consumed, c->block_data, &used);
    if (consumed != len) {
        avro_set_error("Incomplete decompression");
        return 1;
    }
    if (rc != QZ_OK) {
        avro_set_error("Error decompressing block with QAT");
        return 1;
    }
...
static int reset_qat(avro_codec_t c)
{
    if (c->block_data) {
        qzFree(c->block_data);
    }
    ...
    c->used_size = 0;
    if (c->codec_data) {
        qzTeardownSession(codec_data_qz_session(c->codec_data));
        qzClose(codec_data_qz_session(c->codec_data));
        avro_free(c->codec_data, sizeof(struct codec_data_qz));
    }
    c->codec_data = NULL;
...
//TODO: use qat as a meta-type instead of a type
#ifdef QAT_CODEC
    if (strcmp("qat", type) == 0) {
        return codec_qat(codec);
    }
#endif
